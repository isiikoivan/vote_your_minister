<?php
//
//namespace app\modules\CampusSetup\components\Helpers;
//
//use Exception;
//use Mpdf\Mpdf;
...
class ExcelPdfGenerator extends Component
{
     ...
     * @throws \Exception
     */
    public static function generatePdf1($model, $options = [])
    {
        try {
            // Create a new mPDF instance with custom settings
            $mpdf = new Mpdf([
...
//    public static function generateExcel($model, $options = [])
//    {
//        try {
//
...
//                $col = 1; // Start with the first column for each row
//                foreach ($attributes as $key => $header) {
...
//
//            // Set the filename
//            $filename = (isset($options['filename']) ? $options['filename'] : 'export') . '_' . date('Y-m-d_H-i-s') . '.xlsx';
//
//            // Output the Excel file
//            $writer = new Xlsx($spreadsheet);
//
//            // Set headers for the browser to prompt the download
